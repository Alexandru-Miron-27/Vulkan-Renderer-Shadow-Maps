...
layout (set = 1, binding = 3) uniform sampler2D uTexAlphaMap;
...
void main()
{
	...
	//alpha masking discard test
    if (texture(uTexBaseColor, vsTexCoord).a < texture(uTexAlphaMap, vsTexCoord).r) {
        discard;
    } 
	...
	* max(dot(normal, lightDir), 0.0);
	...
	oColor = vec4(Lo, 1.0);
	//oColor = vec4(texture(uNormalMap, vsTexCoord).rgb, 1.0);
	//oColor = vec4(abs(normal), 1.0);
}
