...
namespace labutils
{
	Image load_image_texture2d(char const* aPath, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator, VkFormat aFormat)
	{
		...
		if (auto const res = vkBeginCommandBuffer(cbuff, &beginInfo);
			VK_SUCCESS != res)
		{
		...
		// TRANSFER_DST_OPTIMAL. current layout is UNDEFINED
		...
		//Upload data from staging buffer to iamge
		...
		//transfer base level to transfer_SRC_OPTIMAL
		image_barrier(cbuff, ret.image,
			VK_ACCESS_TRANSFER_WRITE_BIT,
		...
		for (std::uint32_t level = 1; level < mipLevels; ++level)
		{
			...
			//loop starts at level = 1 but level = 0 is the base level that we initialised before the loop
			VkImageBlit blit{};
		...
		//Whole image is currently in the transfer_SRC_OPTIMAL layout
		// to use image as a texture , it must be in the SHADER_READ_ONLY_OPTIMAL
		...
		if (auto const res = vkEndCommandBuffer(cbuff);
			VK_SUCCESS != res)
		{
