...
namespace labutils
{
	...
	VulkanWindow make_vulkan_window()
	{
		...
		for (std::uint32_t i = 0; i < reqExtCount; ++i)
		{
			...
			{
				throw lut::Error("GLFW/Vulkan: required isntance extension %s not supported",
					requiredExt[i]);
			}
...
#		endif // ~ debug builds
		...
		if (auto const res = glfwCreateWindowSurface(ret.instance,
			ret.window,
			nullptr,
			&ret.surface);
			VK_SUCCESS != res)
	...
	SwapChanges recreate_swapchain(VulkanWindow& aWindow)
	{
		...
		//Remember old formats & extents
		//These are two of the properties that may change. 
		//Typically only the extent changes( e.g. window resized), but the format may
		//in theory also change. If the format changes, we need to recreate additional resources
		auto const oldFormat = aWindow.swapchainFormat;
		auto const oldExtent = aWindow.swapchainExtent;

		//Destroy old objects except old swapchain
		...
		//Get new swap chain images & create associate image views
...
namespace
{
	...
	std::unordered_set<VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		return std::unordered_set<VkPresentModeKHR>(modes.begin(), modes.end());
	}
	...
	std::tuple<VkSwapchainKHR, VkFormat, VkExtent2D> create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
	{
		...
		if (std::numeric_limits<std::uint32_t>::max() == extent.width)
		{
			...
			//make sure extent is withing the range defined
		...
		{
			//Multiple queues might accest this resource
			//two alternatives: SHARING_MODE_CONCURRENT allows access without transferring ownership
			//EXCLUSIVE requires explicit ownership transfers (may result in better performance)
	...
	void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages)
	{
		...
		//throw lut::Error( "Not yet implemented!" );
		uint32_t imageCount;
		vkGetSwapchainImagesKHR(aDevice, aSwapchain, &imageCount, nullptr);
		aImages.resize(imageCount);
		vkGetSwapchainImagesKHR(aDevice, aSwapchain, &imageCount, aImages.data());
	}
...
namespace
{
	...
	VkDevice create_device(VkPhysicalDevice aPhysicalDev, std::vector<std::uint32_t> const& aQueues, std::vector<char const*> const& aEnabledExtensions)
	{
		...
		//Enabling anisotropic features
		VkPhysicalDeviceFeatures deviceFeatures{};
		vkGetPhysicalDeviceFeatures(aPhysicalDev, &deviceFeatures);
		VkPhysicalDeviceProperties props;
		vkGetPhysicalDeviceProperties(aPhysicalDev, &props);

		if (deviceFeatures.samplerAnisotropy && props.limits.maxSamplerAnisotropy > 1)
		{
			std::cout << "Max Sampler Anisotropy is: " << props.limits.maxSamplerAnisotropy << std::endl;
			deviceFeatures.samplerAnisotropy = VK_TRUE;
		}
		else
		{
			std::cout << "Anisotropic filtering is not enabled" << std::endl;
		}
