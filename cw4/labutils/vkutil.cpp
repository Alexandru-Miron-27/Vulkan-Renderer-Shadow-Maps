...
namespace labutils
{
	...
	CommandPool create_command_pool(VulkanContext const& aContext, VkCommandPoolCreateFlags aFlags)
	{
			...
			VK_SUCCESS != res)
		{
			throw Error("Unable to create command pool\n"
				"vkCreateCommandPool() returned %s",
				to_string(res).c_str());
		}
	...
	VkCommandBuffer alloc_command_buffer(VulkanContext const& aContext, VkCommandPool aCmdPool)
	{
			...
			VK_SUCCESS != res)
		{
			throw Error("Unable to allocate command buffer\n"
				"vkAllocateCommandBuffers() returned %s",
				to_string(res).c_str());
		}
	...
	Semaphore create_semaphore(VulkanContext const& aContext)
	{
		...
		if (auto const res = vkCreateSemaphore(aContext.device,
			&semaphoreInfo,
			nullptr,
			&semaphore);
			VK_SUCCESS != res)
		...
		std::uint32_t aMaxSets)
	{
			...
			VK_SUCCESS != res)
		{
			throw Error("Unable to create descriptor pool\n"
				"vkCreateDescriptorPool() returned %s",
				to_string(res).c_str());
		}
	...
	VkDescriptorSet alloc_desc_set(VulkanContext const& aContext,
		...
		VkDescriptorSetLayout aSetLayout)
	{
			...
			VK_SUCCESS != res)
		{
			throw Error("Unable to allocate descriptor set\n"
				"vkAllocateDescriptorSets(0 returned %s",
				to_string(res).c_str());
		}
	...
	ImageView create_image_view_texture2D(VulkanContext const& aContext,
		VkImage aImage,
		VkFormat aFormat)
	{
		...
		if (auto const res = vkCreateImageView(aContext.device,
			&viewInfo,
			nullptr,
			&view);
			VK_SUCCESS != res)
	...
	)
	{
		...
		vkCmdPipelineBarrier(aCmdBuff, aSrcStageMask,
			aDstStageMask, 0, 0, nullptr, 0,
			nullptr, 1, &ibarrier);
	}
	...
	Sampler create_default_sampler(VulkanContext const& aContext)
	{
		...
		//enabling anisotropic filtering if it is supported
		...
		//enabling anisotropic filtering if available
		if (features.samplerAnisotropy && props.limits.maxSamplerAnisotropy > 1)
		{
